...
private[wasm] class SymbolsManager {
  ...
  val newFunctions = MMap[Identifier, FunDef]()
  val newRecords = MMap[Identifier, RecordSort]()
...
class ExprTransformer
    (manager: SymbolsManager, keepContracts: Boolean)
    (implicit sym0: stainless.trees.Symbols, sym: wasm.trees.Symbols)
  extends Transformer
{
  import t._
...
trait RecordAbstractor extends inox.transformers.SymbolTransformer with Transformer {
  ...
  def transform(syms: s.Symbols): t.Symbols = {
    ...
    val tr = new ExprTransformer(manager, true)(syms, initSymbols)
