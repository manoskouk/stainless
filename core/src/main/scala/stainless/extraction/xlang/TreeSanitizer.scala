...
trait TreeSanitizer {
  ...
  /** Throw a [[MisformedStainlessCode]] exception when detecting an illegal pattern. */
  def check(symbols: Symbols)(implicit ctx: inox.Context): Unit = {
    ...
    } throw MisformedStainlessCode(symbols.getFunction(id),
    ...
    } throw MisformedStainlessCode(cd, "Illegal sealed abstract class with no children.")
  }
  ...
  private class CheckPreconditions(implicit symbols: Symbols, ctx: inox.Context) extends SelfTreeTraverser {
    ...
    override def traverse(e: Expr): Unit = e match {
      ...
      case e: Require =>
        throw MisformedStainlessCode(e, s"Unexpected `require`.")

      case e: Decreases =>
        throw MisformedStainlessCode(e, s"Unexpected `decreases`.")
    ...
    override def traverse(e: Expr): Unit = e match {
      case ClassSelector(obj, selector) =>
        ...
        ct.getField(selector) match {
          case None =>
            throw MisformedStainlessCode(e, s"Cannot find field `${selector.asString}` of class ${ct.asString}.")
          case Some(field) if field.flags contains Ignore =>
            throw MisformedStainlessCode(e, s"Cannot access ignored field `${selector.asString}` from non-extern context.")
          case _ =>
            super.traverse(e)
        }
        ...
        symbols.getFunction(id).flags collectFirst { case IsAccessor(Some(id)) => id } match {
          case Some(id) =>
            ...
            ct.getField(id) match {
              case Some(field) if field.flags contains Ignore =>
                throw MisformedStainlessCode(e, s"Cannot access ignored field `${id.asString}` from non-extern context.")
      ...
      case ClassConstructor(ct, args) =>
        ct.lookupClass match {
          case None =>
            throw MisformedStainlessCode(e, s"Cannot find class for type `${ct.asString}`.")
          ...
          case Some(tcd) if tcd.fields.exists(_.flags contains Ignore) =>
            ...
            throw MisformedStainlessCode(e,
